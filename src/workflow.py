...
class AstroWorkflow:
    ...
    def create_session(self, session_id: str, user_input: str, 
                      user_context: Optional[Dict[str, Any]] = None) -> AstroAgentState:
        """
        创建新的用户会话
    ...
    def execute_workflow(self, session_id: str, user_input: str, 
                        user_context: Optional[Dict[str, Any]] = None) -> AstroAgentState:
        """
        执行完整的工作流程
        ...
        try:
            ...
            if session_id not in self.sessions:
                initial_state = self.create_session(session_id, user_input, user_context)
            else:
                # 更新现有会话
    ...
    def _log_execution_result(self, session_id: str, final_state: AstroAgentState, 
        ...
        if history:
            ...
            for i, step in enumerate(history[-3:], 1):  # 只显示最后3步
                logger.info(f"    {i}. {step.get('node', '未知')}: {step.get('action', '未知')}")
...
def execute_astro_workflow(session_id: str, user_input: str, 
                          user_context: Optional[Dict[str, Any]] = None,
                          config_path: Optional[str] = None) -> AstroAgentState:
    """
    便捷函数：执行天文科研Agent工作流
...
if __name__ == "__main__":
    ...
    for i, test_input in enumerate(test_cases, 1):
        ...
        try:
            ...
            print(f"结果: {result.get('current_step', '未知')} - {result.get('user_type', '未识别')}")
        except Exception as e:
            print(f"错误: {str(e)}")